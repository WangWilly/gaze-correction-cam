...
class gaze_redirection_system:
    ...
    def monitor_para(self, frame, fig_alpha, fig_eye_pos, fig_R_w):
        ...
        cv2.putText(
            frame,
            "Eye:["
            + str(int(fig_eye_pos[0]))
            + ","
            + str(int(fig_eye_pos[1]))
            + ","
    ...
    def shifting_angles_estimator(self, R_le, R_re, shared_v, lock):
        ...
        try:
            ...
            tar_win = next(
                (win for win in window_list if win.get("kCGWindowName") == "Remote"),
                None,
            )
            if tar_win:
                Rw_lt = (
                    tar_win["kCGWindowBounds"]["X"],
                    tar_win["kCGWindowBounds"]["Y"],
                )
                size_window = (
                    tar_win["kCGWindowBounds"]["Width"],
                    tar_win["kCGWindowBounds"]["Height"],
                )
            else:
    ...
    def flx_gaze(
        ...
        for k, bx in enumerate(detections):
            ...
            # get eye
            LE_img, LE_M_A, LE_center, size_le_ori, R_le_LT = self.get_inputs(
                frame, shape, pos="L", size_I=size_I
            )
            RE_img, RE_M_A, RE_center, size_re_ori, R_re_LT = self.get_inputs(
                frame, shape, pos="R", size_I=size_I
            )
            # shifting angles estimator
            alpha_w2c, p_e, R_w = self.shifting_angles_estimator(
            ...
            # repace eyes
            frame[
                (R_le_LT[0] + pixel_cut[0]) : (
                    R_le_LT[0] + size_le_ori[0] - pixel_cut[0]
                ),
                (R_le_LT[1] + pixel_cut[1]) : (
                    R_le_LT[1] + size_le_ori[1] - pixel_cut[1]
                ),
            ] = (
                LE_infer[
                    pixel_cut[0] : (-1 * pixel_cut[0]),
                    pixel_cut[1] : -1 * (pixel_cut[1]),
                ]
                * 255
            )
            frame[
                (R_re_LT[0] + pixel_cut[0]) : (
                    R_re_LT[0] + size_re_ori[0] - pixel_cut[0]
                ),
                (R_re_LT[1] + pixel_cut[1]) : (
                    R_re_LT[1] + size_re_ori[1] - pixel_cut[1]
                ),
            ] = (
                RE_infer[
                    pixel_cut[0] : (-1 * pixel_cut[0]),
    ...
    def redirect_gaze(self, frame, shared_v, lock):
        ...
        rg_thread = Thread(
            target=self.flx_gaze, args=(frame, gray, detections, shared_v, lock)
        )
        rg_thread.start()
        return True
