...
def trans_module(inputs, structures, phase_train, name="trans_module"):
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        cnn_blk_2 = tf_utils.cnn_blk(
            tf.concat([cnn_blk_0, cnn_blk_1], axis=3),
            structures["depth"][2],
            structures["filter_size"][2],
            phase_train,
            name="cnn_blk_2",
        )
        cnn_blk_3 = tf_utils.cnn_blk(
            tf.concat([cnn_blk_0, cnn_blk_1, cnn_blk_2], axis=3),
            structures["depth"][3],
    ...
    with tf.compat.v1.variable_scope("warping_model"):
        ...
        with tf.compat.v1.variable_scope("warping_module"):
            ...
            fine_out = trans_module(
                fine_input, fine_layer, phase_train, name="fine_level"
            )
...
def dist_loss(y_pred, y_, method="MAE"):
    with tf.compat.v1.variable_scope("img_dist_loss"):
        ...
        if method == "L2":
            loss = tf.sqrt(
                tf.reduce_sum(tf.square(y_pred - y_), axis=3, keep_dims=True)
            )
        elif method == "MAE":
...
def center_weight(shape, base=0.005, boundary_penalty=3.0):
    with tf.compat.v1.variable_scope("center_weight"):
        ...
        dist2cent = (
            temp * tf.sqrt(tf.reduce_sum(tf.square(tf.concat([X, Y], axis=2)), axis=2))
            + base
        )
        dist2cent = tf.expand_dims(
            tf.tile(tf.expand_dims(dist2cent, axis=0), [shape[0], 1, 1]), axis=3
        )
        return dist2cent
...
def loss(img_pred, img_, eye_mask, input_img, flow, lcm_wgt):
    with tf.compat.v1.variable_scope("losses"):
        ...
        loss_eyeball, loss_eyelid, loss_lcm = TVlosses(
            eye_mask, input_img, flow, lcm_wgt
        )
